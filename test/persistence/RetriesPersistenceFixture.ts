...
export class RetriesPersistenceFixture {
    ...
    public testGetRetries(done) {
        ...
        async.series([
            ...
            (callback) => {
                this._persistence.getPageByFilter(null, FilterParams.fromTuples("collection", "c2"), new PagingParams(0, 10, false), (err, page) => {
                    assert.isNull(err);
    ...
    public testGetRetry(done) {
        async.series([
            ...
            (callback) => {
                // Test retries
                this._persistence.getById(null, TestModel.DUPLICATE1.collection, TestModel.DUPLICATE1.id, (err, retry) => {
                    assert.isNull(err);
    ...
    public testRetry(done) {
        async.series([
            ...
            (callback) => {
                ...
                this._persistence.getById(null, TestModel.DUPLICATE2.collection, TestModel.DUPLICATE2.id, (err, retry) => {
                    assert.isNull(err);
            ...
            (callback) => {
                ...
                this._persistence.getById(null, TestModel.DUPLICATE2.collection, "4", (err, retry) => {
                    assert.isNull(err);
                    assert.isNull(retry);
                    callback();
                });
                ...
                // Delete retry
                this._persistence.delete(null, TestModel.DUPLICATE3.collection, TestModel.DUPLICATE3.id, (err) => {
                    assert.isNull(err);
                    this._persistence.getById(null, TestModel.DUPLICATE3.collection, TestModel.DUPLICATE3.id, (e, retry) => {
                        assert.isNull(e);
                        assert.isNull(retry);
                        callback();
                    });
